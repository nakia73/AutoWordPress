...
**目的:**
複数クライアントのWordPressサイトを効率的に管理するため、「WordPress Multisite」アーキテクチャの妥当性を評価し、技術的な設計を行う。
...
| 特徴                  | WordPress Multisite                 |
| :-------------------- | :---------------------------------- |
| **アーキテクチャ**    | 単一のアプリ・DBで論理分割          |
| **リソース効率**      | 高（共有メモリ + キャッシュ）       |
| **保守 (Update)**     | 1回で全サイト適用                   |
| **隔離性 (Security)** | 中（プラグイン/テーマは共有、権限で制御） |
| **構築速度**          | サイト作成 0.1秒                    |
| **Exit Strategy**     | XMLエクスポート/Migrationプラグイン |
...
- **結論:** 標準機能で十分に移行可能であり、**ロックインにはならない**。
