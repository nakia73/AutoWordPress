...
# 補遺: SPEC_REVIEW_REPORT からの追加項目

> **追記日:** 2026年1月26日
> **出典:** SPEC_REVIEW_REPORT.md（2026年1月26日作成・Ralph Wiggum Loop分析）
> **注記:** 以下の項目は本レポート（Part 1-5）に明示的に含まれていない追加のリスク・懸念事項です。

---

## SR-001: LiteLLMプロキシの構成未定義（重要度: 中）

**発見箇所:** `architecture/04_AI_Pipeline.md`

**問題:**
- 「LiteLLMプロキシ経由」と記載があるが、具体的な構成が未定義
- ホスティング場所（Vercel、VPS、別サービス）が不明
- Vercelでの運用はタイムアウト制限に引っかかる可能性
- VPSにデプロイすると運用負荷が増加

**対応方針:**
```
オプション:
A) Vercel Functionsで直接Gemini APIを呼び出し（LiteLLM不要）
B) VPS上にLiteLLMをデプロイ（追加運用負荷）
C) LiteLLMプロキシサービス（Portkey等）を利用

推奨: MVP段階ではオプションA（直接呼び出し）
```
...
## SR-002: WordPress管理画面へのアクセス制御（重要度: 中）

**発見箇所:** `architecture/03_Infrastructure_Ops.md`

**問題:**
- 「管理画面へのIP制限（オプション）」とされているが、SaaS環境では必須と考えるべき
- ユーザーがWP管理画面にアクセスする場合のセキュリティ設計が不十分

**対応方針:**
```
MVP:
- Phase 9（SSO）までは管理画面へのアクセスを制限
- ユーザーはダッシュボード経由でのみ操作
...
## SR-003: Cloudflare R2移行タイミング（重要度: 低）

**発見箇所:** `architecture/00_Master_Architecture.md`

**問題:**
- Cloudflare R2をメディアストレージとして使用と記載
- MVPからR2を使用するのか、後から移行するのかが不明確

**対応方針:**
```
MVP: ローカルストレージ（VPS上の/var/www/html/wp-content/uploads）
Phase 7以降: Cloudflare R2への移行（オフロードプラグイン使用）
...
## SR-004: Phase Dの競合分析の深度（重要度: 中）

**発見箇所:** `architecture/04_AI_Pipeline.md`

**問題:**
- Tavily Searchで取得できるのはスニペット程度
- 競合記事の全文を取得してLLM分析するには追加でスクレイピングが必要
- コスト・時間のトレードオフが考慮されていない

**対応方針:**
```
MVP: スニペットのみで分析（Tavily結果をそのまま使用）
成長フェーズ: Firecrawl/Jina Readerで上位3記事の全文取得

Phase D処理フロー（成長フェーズ）:
1. Tavily Search → 上位10件のURL取得
2. 上位3件をFirecrawl/Jina Readerで全文取得
3. LLMで競合分析実施
```

**対応期限:** Phase 2 開始前に方針確定

---

## SR-005: PESスコア計算式の妥当性（重要度: 中）

**発見箇所:** `phases/Phase15_PromptIntelligence.md`
...
- インプレッションとクリックは絶対数、CTRは比率であり、単位が異なる指標を単純に加算
- 正規化処理が必要
...
type NormalizedMetrics = {
  impressions: number;  // 0-100スケール（percentile）
  clicks: number;       // 0-100スケール（percentile）
  position: number;     // 0-100スケール（1位=100, 100位=0）
  ctr: number;          // 0-100スケール（CTR%を直接使用）
};

function calculatePES(metrics: NormalizedMetrics): number {
  return (
    metrics.impressions * 0.2 +
    metrics.clicks * 0.3 +
    metrics.position * 0.3 +
    metrics.ctr * 0.2
  );
}
```
...
## SR-006: 統計的有意性の判定方法（重要度: 中）

**発見箇所:** `phases/Phase15_PromptIntelligence.md`

**問題:**
- 「統計的有意性: 95%」と記載があるが、具体的な検定方法が未定義
- t検定、カイ二乗検定、ベイズ推定など、どれを採用するか不明
...
- 主指標（CTR）: 二項比率のz検定
- 副指標（滞在時間）: t検定
- 最小サンプルサイズ: 各バリアント100インプレッション
- 有意水準: α = 0.05

実装:
- 自前実装ではなくLangfuse/Growthbook等のA/Bテストツールを使用
```
...
## SR-007: プライバシー対応の詳細（重要度: 低）

**発見箇所:** `phases/Phase15_PromptIntelligence.md`

**問題:**
- 「オプトアウト可能」と記載があるが、具体的なUI/UXフローが未定義
- GDPRやCCPA対応の詳細が不明

**対応方針:**
```
オプトアウトフロー:
1. ダッシュボード設定画面に「データ共有設定」セクション
2. 「プロンプト効果分析への参加」トグル
3. オプトアウト時: article_generation_logs への記録を匿名化
...
GDPR対応:
- データエクスポート機能（Phase 3で実装）
- データ削除機能（Phase 5で実装）
```

**対応期限:** Phase 3（ダッシュボード）実装時

---

## SR-008: フロントエンド詳細設計の不足（重要度: 低）

**発見箇所:** `architecture/01_Frontend_Architecture.md`

**問題:**
- ディレクトリ構造は定義されているが、具体的なコンポーネント設計、画面遷移図が不足
...
1. ランディングページ（LP）
2. サインイン/サインアップ
3. オンボーディング（プロダクトURL入力）
4. ダッシュボード（記事一覧、ステータス表示）
5. 記事詳細（プレビュー、公開/下書き切り替え）
6. 設定（プロフィール、請求情報）
...
- [ ] SR-001: LiteLLM使用方針を確定
- [ ] SR-002: 管理画面アクセス制御方針を確定
- [ ] SR-003: R2移行タイミング方針を確定

### Phase 2 前

- [ ] SR-004: Phase D競合分析の深度を確定

### Phase 3 前

- [ ] SR-007: プライバシー設定UIを設計
- [ ] SR-008: フロントエンド画面遷移図を作成

### Phase 15 前

- [ ] SR-005: PESスコア計算式を正規化
- [ ] SR-006: A/Bテスト統計検定方法を確定
...
| 日付 | 内容 |
|------|------|
| 2026-01-26 | Part 1-5 作成（89件の問題特定） |
| 2026-01-26 | 補遺追加：SPEC_REVIEW_REPORTからの追加項目8件（SR-001〜SR-008） |
